...
function renderSidebarAppointments() { 
    ...
    misCitas.forEach(cita => { 
        ...
        div.innerHTML = ` <strong>${cita.especialidad}</strong><br> <small class="text-muted">${cita.medico}</small><br> <small class="text-secondary">${cita.fechaHora.toLocaleString()}</small> <button class="btn btn-danger btn-sm w-100 mt-1" onclick="intentarCancelarCita(${cita.id})">Cancelar</button> `; 
        container.appendChild(div); 
    });
}
...
// ====================================================
// DETECTOR AUTOMÁTICO DE CAMBIOS Y SISTEMA DE GUARDADO
// ====================================================

document.addEventListener("DOMContentLoaded", () => {
    // Monitorea cambios visuales dentro del panel del médico
    const vistaMedico = document.getElementById("view-medico");
    const botonGuardar = document.getElementById("btnGuardar");

    if (vistaMedico && botonGuardar) {
        // Muestra el botón si escriben texto
        vistaMedico.addEventListener("input", () => {
            botonGuardar.style.display = "block";
        });

        // Muestra el botón si cambian un selector o checkbox
        vistaMedico.addEventListener("change", () => {
            botonGuardar.style.display = "block";
        });
    }
});
...
function guardarHorarios() {
    ...
    // Ocultar botón tras confirmar el guardado exitoso
    const botonGuardar = document.getElementById("btnGuardar");
    if (botonGuardar) {
